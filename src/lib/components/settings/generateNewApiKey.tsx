...
export default function GenerateNewApiKey({
  cancelAction,
}: GenerateNewApiKeyProps) {
  //@ts-ignore
  const [newApiKeyState, formAction] = useFormState(
  //@ts-ignore
    generateNewApiKeyFormAction,
    initialState,
  );
    ...
    <div>
      ...
      <form action={formAction} className='flex flex-col'>
        <div className='mb-3 mt-3 flex flex-col'>
          <label className='text-md mb-1 font-semibold' htmlFor='input-label'>
            Token name
          </label>
          <input
        ...
        <div className='mb-3 mt-1 flex flex-col'>
          ...
          <TextareaAutosize
            ...
            className='text-md input input-bordered my-auto min-h-7 max-w-96 resize-none overflow-y-auto overflow-x-hidden rounded-sm border-gray-200 focus:outline-none'
          />
          <p className='text-sm text-gray-600'>
        ...
        <div className='mb-3 flex flex-col'>
          ...
          <input
            name='expiration'
            type='hidden'
            value={addMonthsToDate(Number(expiration))
              .toISOString()
              .substring(0, 10)}
          />
          <FormDropdown
    ...
    <div className='flex flex-col pr-3'>
      ...
      <div className='flex flex-row justify-between border-b-[.5px] border-gray-200 pb-2'>
        <button
          ...
          className='mb-[2px] min-w-10 rounded-lg border-[.5px] border-gray-300 p-2 text-center text-sm font-normal transition duration-75 ease-in hover:bg-gray-100 dark:hover:bg-gray-700'
        >
          Generate new key
        </button>
        <button
      ...
      <ul>
          ...
          <div className='flex flex-col rounded-lg border-[.5px] border-gray-200 bg-green-200 p-2'>
            <div className='flex flex-row items-center justify-between'>
              <div className='flex flex-row items-center gap-1'>
                ...
                <h3 className='text-gray-600'>{newApiKeyState.res?.apikey}</h3>
                <button
                  onClick={() => {
                    notifyInfo('copied to clipboard');
                    navigator.clipboard.writeText(
                      newApiKeyState.res?.apikey as string,
                    );
                  }}
                  className=''
                >
                  <Copy size={15} color='#338ba8' />
                </button>
              </div>
              <button
                className='mb-[2px] h-10 min-w-10 rounded-lg border-[.5px] border-red-500 p-2 text-center text-sm font-semibold text-red-400 transition duration-75 ease-in hover:bg-red-400 hover:text-white'
                onClick={async () =>
                  setApiKeyRevokeRequest(
                    await revokeApiKey(newApiKeyState.res?.id as any),
                  )
                }
              >
                Revoke
        ...
        <ApiKeyDisplay hideKeyId={newApiKeyState.res?.id} />
      </ul>
      <ToastContainer
